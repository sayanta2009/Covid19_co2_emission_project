...
from keras.layers import Flatten, MaxPooling2D, Conv2D, ZeroPadding2D
...
class cnn_model_2:
    ...
    def make_cnn_model(self, train, main_model):
        """
        This method is used to restructure our data required to feed into CONV2D models. 'train' is the flag to
        ...
        if not main_model:
            for feature_name in self.data.columns:
                if feature_name not in ('Year', 'Country', 'Country_Code', 'Energy_CO2_Emissions',
                    ...
                    self.model_name = 'src/Models/model_checkpoints/cnn_2/model_cnn_2_' + self.feature_name
                    if train:
            ...
            self.x_main_model = self.x_main_model.reshape(self.x_main_model.shape[0], self.x_main_model.shape[1],
                                                          self.x_main_model.shape[2], 1)
    ...
    def train_cnn_model(self, x_train, y_train):
        """
        This method is used to train CONV2D models given training data
        """
    ...
    def build_cnn_features_model(self):
        """
        Builds CONV2D models for features
        """
        self.model = keras.Sequential()
        self.model.add(ZeroPadding2D(padding=(2, 2), input_shape=(self.x.shape[1], self.x.shape[2], self.x.shape[3])))
        self.model.add(Conv2D(filters=64, kernel_size=(3, 3), activation='relu', padding='valid'))
    ...
    def build_cnn_main_model(self):
        """
        Builds CONV2D model for the main CO2 emission data.
        """
        self.model = keras.Sequential()
        self.model.add(Conv2D(filters=64, kernel_size=(3, 3), activation=RELU_ACTIVATION, padding='same',
                              input_shape=(
                                  self.x_main_model.shape[1], self.x_main_model.shape[2], self.x_main_model.shape[3])))
        self.model.add(MaxPooling2D((2, 2)))
        self.model.add(Flatten())
        self.model.add(Dense(128, activation=RELU_ACTIVATION, name="Hidden_Layer_1"))
        self.model.add(Dense(64, activation=RELU_ACTIVATION, name="Hidden_Layer_2"))
        ...
        self.model.add(Dense(NUMBER_OF_COUNTRIES, activation=LINEAR_ACTIVATION))
        self.model.compile(loss=root_mean_squared_error, optimizer=ADAM_OPTIMIZER, metrics=[r2_keras])
        # plot_model(self.model, show_shapes=True, show_layer_names=True, dpi=128, to_file=CONV2D_MAIN_MODEL_STRUCTURE)
    ...
    def test_cnn_model(self, x_test, y_test, main_model):
        """
        This method is used to evaluate the conv2D models. 'main_model' is the flag which signifies whether we are
        evaluating for the main model or the feature models
        """

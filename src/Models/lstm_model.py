...
class Lstm:
    ...
    def make_lstm_model(self, train, main_model):
        """
        This method is used to restructure our data required to feed into Lstm models. 'train' is the flag to
        ...
        if not main_model:
            for feature_name in self.data.columns:
                if feature_name not in ('Year', 'Country', 'Country_Code', 'Energy_CO2_Emissions',
                    ...
                    util = ModelUtil(self.data, NUMBER_OF_YEARS, feature_name=feature_name)
                    ...
                    self.build_lstm_features_model()
                    print(self.model.summary())
                    self.model_name = 'src/Models/model_checkpoints/lstm/model_lstm_' + self.feature_name
                    if train:
                        self.train_lstm_model(x_train, y_train)
                    else:
                        self.test_lstm_model(x_test, y_test, False)
        else:
            ...
            y_train = y_train[:, :, 0]
            self.build_lstm_main_model()
            print(self.model.summary())
            self.model_name = 'src/Models/model_checkpoints/lstm/model_lstm_' + self.feature_name
            if train:
                self.train_lstm_model(x_train, y_train)
            else:
                self.test_lstm_model(x_test, y_test, True)

        return self.r2_score

    def train_lstm_model(self, x_train, y_train):
        """
        This method is used to train Lstm models given training data
        """
    ...
    def build_lstm_features_model(self):
        """
        Builds Lstm models for features
        """
        self.model = keras.Sequential()
        self.model.add(LSTM(units=HIDDEN_LAYER_NEURONS, return_sequences=True,
                            input_shape=(self.x.shape[1], self.x.shape[2])))
        self.model.add(Dropout(DROPOUT_RATE, name='Dropout_Layer_1'))
        self.model.add(LSTM(units=HIDDEN_LAYER_NEURONS, activation=RELU_ACTIVATION))
        self.model.add(Dropout(DROPOUT_RATE, name='Dropout_Layer_2'))
        ...
        self.model.add(Dense(units=NUMBER_OF_COUNTRIES, activation=LINEAR_ACTIVATION, name='Output_Layer'))
        self.model.compile(optimizer=ADAM_OPTIMIZER, loss=root_mean_squared_error, metrics=[r2_keras])
        plot_model(self.model, show_shapes=True, show_layer_names=True, dpi=128, to_file=LSTM_FEATURE_MODEL_STRUCTURE)

    def build_lstm_main_model(self):
        """
        Builds Lstm model for the main CO2 emission data.
        """
        self.model = keras.Sequential()
        self.model.add(LSTM(units=HIDDEN_LAYER_NEURONS_MAIN_MODEL, return_sequences=True,
                            input_shape=(self.x_main_model.shape[1], self.x_main_model.shape[2])))
        self.model.add(Dropout(DROPOUT_RATE))
        self.model.add(LSTM(units=HIDDEN_LAYER_NEURONS_MAIN_MODEL, activation=RELU_ACTIVATION))
        self.model.add(Dropout(DROPOUT_RATE))
        self.model.add(Dense(128, activation=RELU_ACTIVATION, name="Hidden_Layer_1"))
        self.model.add(Dense(64, activation=RELU_ACTIVATION, name="Hidden_Layer_2"))
        self.model.add(Dense(16, activation=RELU_ACTIVATION, name="Hidden_Layer_3"))
        self.model.add(Dense(units=NUMBER_OF_COUNTRIES, activation=LINEAR_ACTIVATION))
        self.model.compile(optimizer=ADAM_OPTIMIZER, loss=root_mean_squared_error, metrics=[r2_keras])
        # plot_model(self.model, show_shapes=True, show_layer_names=True, dpi=128, to_file=LSTM_MAIN_MODEL_STRUCTURE)
    ...
    def test_lstm_model(self, x_test, y_test, main_model):
        """
        This method is used to evaluate the Lstm models. 'main_model' is the flag which signifies whether we are
        evaluating for the main model or the feature models
        """

...
os.chdir('../../')
feature_name = 'Harvested_Wood_Products'
...
param_grid = dict(activation=activation_fn, filters=filter_set,
                  optimizer=optimizers, batch_size=batch_size, epochs=epochs)
if not evaluation:
    if search:
        ...
        np.save('src/Models/model_checkpoints/train_history/harvested_wood_products.npy', train_history.history)
        np.save('src/Models/model_checkpoints/train_history/harvested_wood_products_best_params.npy', grid_result.best_params_)
    else:
        best_parameters = np.load('src/Models/model_checkpoints/train_history/harvested_wood_products_best_params.npy',
        ...
        mc = ModelCheckpoint('src/Models/model_checkpoints/best_models/best_model_Harvested_Wood_Products',
                             monitor='val_loss', verbose=1, save_best_only=True, mode='min')
        callbacks_list = [es, mc]
        train_history = best_model.fit(x_train, y_train, epochs=best_epochs, batch_size=best_batch_size,
                                       validation_split=VALIDATION_SPLIT_SIZE, callbacks=callbacks_list, verbose=1)
        best_model.save('src/Models/model_checkpoints/best_models/best_model_Harvested_Wood_Products')

else:
    tuned_model = load_model('src/Models/model_checkpoints/best_models/best_model_Harvested_Wood_Products',

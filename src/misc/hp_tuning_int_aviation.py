...
os.chdir('../../')
feature_name = 'International_Aviation'
...
param_grid = dict(activation=activation_fn, filters=filter_set, optimizer=optimizers, batch_size=batch_size, epochs=epochs)
...
if not evaluation:
    if search:
        ...
        np.save('src/Models/model_checkpoints/train_history/int_aviation.npy', train_history.history)
        np.save('src/Models/model_checkpoints/train_history/int_aviation_best_params.npy', grid_result.best_params_)
    else:
        best_parameters = np.load('src/Models/model_checkpoints/train_history/int_aviation_best_params.npy',
        ...
        mc = ModelCheckpoint('src/Models/model_checkpoints/best_models/best_model_International_Aviation',
                             monitor='val_loss', verbose=1, save_best_only=True, mode='min')
        callbacks_list = [es, mc]
        train_history = best_model.fit(x_train, y_train, epochs=best_epochs, batch_size=best_batch_size,
                                       validation_split=VALIDATION_SPLIT_SIZE, callbacks=callbacks_list, verbose=1)
        best_model.save('src/Models/model_checkpoints/best_models/best_model_International_Aviation')

else:
    tuned_model = load_model('src/Models/model_checkpoints/best_models/best_model_International_Aviation',

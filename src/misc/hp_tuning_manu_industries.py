...
from keras.layers import Dense, Flatten, MaxPooling2D, Conv2D, ZeroPadding2D
import keras
import numpy as np
...
os.chdir('../../')
feature_name = 'Manufacturing_Industries'
...
def build_cnn2_model(filters=64, optimizer=ADAM_OPTIMIZER, activation='relu'):

    model = keras.Sequential()
    model.add(ZeroPadding2D(padding=(2, 2), input_shape=(x_train.shape[1], x_train.shape[2], x_train.shape[3])))
    model.add(Conv2D(filters=filters, kernel_size=(3, 3), activation='relu', padding='valid'))
...
if not evaluation:
    if search:
        reg_model = KerasRegressor(build_fn=build_cnn2_model, verbose=1)
        ...
        best_model = build_cnn2_model(best_filters, best_optimizer, best_activation)
        ...
        np.save('src/Models/model_checkpoints/train_history/manu_industries.npy', train_history.history)
        np.save('src/Models/model_checkpoints/train_history/manu_industries_best_params.npy', grid_result.best_params_)
    else:
        best_parameters = np.load('src/Models/model_checkpoints/train_history/manu_industries_best_params.npy',
        ...
        mc = ModelCheckpoint('src/Models/model_checkpoints/best_models/best_model_Manufacturing_Industries',
                             monitor='val_loss', verbose=1, save_best_only=True, mode='min')
        callbacks_list = [es, mc]
        train_history = best_model.fit(x_train, y_train, epochs=best_epochs, batch_size=best_batch_size,
                                       validation_split=VALIDATION_SPLIT_SIZE, callbacks=callbacks_list, verbose=1)
        best_model.save('src/Models/model_checkpoints/best_models/best_model_Manufacturing_Industries')
else:
    tuned_model = load_model('src/Models/model_checkpoints/best_models/best_model_Manufacturing_Industries',
